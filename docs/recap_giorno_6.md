...
- [x] Implementare per la rotta /signup la validazione del body con libreria `joi` (schema con username,email,password, repeat_password, email )
...
- [x] Implementare la rotta /login 
- [x] Implementare per la rotta /login la validazione del body con libreria `joi` (schem con email e password)
- [x] Implementare query per cercare l utente tramite email nel db
- [x] Comparare con `bcrypt` password in plaintext con la password_hashata
- [x] Integrare la generazione del token JWT.
